...
# [Hyrax-overwrite-v3.3.0] FileActor ingest_file method in Hyrax::Actors
...
Hyrax::Actors::FileActor.class_eval do
  ...
  def ingest_file(io)
    ...
    pathhint = pathhint(io)
    # Perform characterize job only on preservation_master_file
    CharacterizeJob.perform_later(file_set, repository_file.id, pathhint) if relation == :preservation_master_file
    file_derivatives(file_set, pathhint, repository_file) if io.preferred == io.relation
  end
