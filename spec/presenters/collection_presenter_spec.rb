...
RSpec.describe Hyrax::CollectionPresenter do
  ...
  describe "#total_items", :clean_repo do
    ...
    context "collection with works" do
      let(:collection) { FactoryBot.create(:collection_lw) }
      let!(:work) { FactoryBot.create(:collection_lw, member_of_collection_ids: [collection.id]) }
  ...
  describe "#total_viewable_items", :clean_repo do
    ...
    context "collection with private collection" do
      let!(:work) { FactoryBot.build(:private_collection_lw, member_of_collections: [collection]) }
  ...
  describe "#parent_collection_count" do
    ...
    context('when parent_collections is has collections') do
      let(:collection1) { FactoryBot.build(:collection_lw, title: ['col1']) }
      let(:collection2) { FactoryBot.build(:collection_lw, title: ['col2']) }
