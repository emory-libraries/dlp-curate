...
RSpec.describe FileSet, :perform_enqueued, :clean do
  ...
  describe 'noid integration' do
    context 'when noid does not exist' do
      ...
      let(:file_set1)    { FactoryBot.build(:file_set) }
      ...
      context "after saving" do
        ...
        it 'returns the expected identifier' do
          expect(file_set1.id).to eq noid
        end
      end
    end
    ...
    context 'when noid exists' do
      ...
      let(:file_set2)    { FactoryBot.build(:file_set) }
      ...
      context "after saving" do
        before do
          expect(file_set2.id).not_to eq noid2
          file_set2.assign_id
          file_set2.save!
        end

        it 'returns the expected identifier' do
          expect(file_set2.id).to eq noid2
        end
      end
    end
  end
