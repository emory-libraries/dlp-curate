...
module Bulkrax
  ...
  module ParserExportRecordSet
    ...
    def self.for(parser:, export_from:)
      "Bulkrax::ParserExportRecordSet::#{export_from.classify}".constantize.new(parser:)
    end
          ...
          @complete_entry_identifiers ||=
            begin
              ...
              complete_statuses ||= Bulkrax::Status.latest_by_statusable
                                                   .includes(:statusable)
                                                   .where(
                                                      'bulkrax_statuses.statusable_id IN (?) AND bulkrax_statuses.statusable_type = ? AND status_message = ?',
                                                      entry_ids,
                                                      'Bulkrax::Entry',
                                                      'Complete'
                                                    )
