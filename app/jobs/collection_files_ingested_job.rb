...
class CollectionFilesIngestedJob < Hyrax::ApplicationJob
  def perform(collection_array = nil)
    ...
    collection_hashes = collections.map do |hsh|
      ...
      {
        "collection_id" =>    hsh[:id],
        "collection_title" => hsh[:title]&.first,
        "work_total" =>       works.size,
        "fileset_total" =>    filesets.size,
        "file_total" =>       filesets_file_count(filesets)
      }
    end
    File.open(Rails.root.join('public', "collection-counts-for-#{Time.now.strftime("%Y%m%dT%H%M")}.json"), "w") do |f|
