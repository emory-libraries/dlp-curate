...
module Bulkrax
  ...
  module ImportBehavior
    ...
    def add_user_to_permission_templates!
      ...
      factory.add_user_to_collection_permissions(collection: @item, user:)
    end

    def parent_jobs
      parsed_metadata[related_parents_parsed_mapping].each do |parent_identifier|
        next if parent_identifier.blank?

        PendingRelationship.create!(child_id: identifier, parent_id: parent_identifier, importer_run_id: importerexporter.last_run.id, order: id)
      end
    end

    def child_jobs
      parsed_metadata[related_children_parsed_mapping].each do |child_identifier|
        next if child_identifier.blank?

        PendingRelationship.create!(parent_id: identifier, child_id: child_identifier, importer_run_id: importerexporter.last_run.id, order: id)
      end
    end
    ...
    def add_visibility
      parsed_metadata['visibility'] = importerexporter.visibility if parsed_metadata['visibility'].blank?
    end
    ...
    def add_collections
      ...
      find_collection_ids.each_with_index do |c, i|
        parsed_metadata['member_of_collections_attributes'][i.to_s] = { id: c }
      end
    end
