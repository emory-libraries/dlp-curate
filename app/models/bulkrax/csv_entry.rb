  ...
  class CsvEntry < Entry
    ...
    class CsvWrapper
      ...
      def each
        @original.each do |row|
          next if all_fields_are_empty_for(row:)
          yield(row)
        end
      end
    ...
    def add_file
      ...
      if record['file']&.is_a?(String)
        parsed_metadata['file'] = record['file'].split(Bulkrax.multi_value_element_split_on)
      elsif record['file'].is_a?(Array)
        parsed_metadata['file'] = record['file']
      end
      parsed_metadata['file'] = parsed_metadata['file'].map do |f|
        next if f.blank?
        ...
        obj.each_key do |key|
          if obj[key].is_a?(Array)
            obj[key].each_with_index do |_nested_item, nested_index|
              parsed_metadata["#{key_for_export(key)}_#{index + 1}_#{nested_index + 1}"] = prepare_export_data(obj[key][nested_index])
            end
          else
            parsed_metadata["#{key_for_export(key)}_#{index + 1}"] = prepare_export_data(obj[key])
          end
        end
      end
    end
    ...
    def find_collection_ids
      return collection_ids if collections_created?
      if collection_identifiers.present?
        collection_identifiers.each do |collection_id|
          c = find_collection(collection_id)
          skip = c.blank? || collection_ids.include?(c.id)
          collection_ids << c.id unless skip
        end
      end
