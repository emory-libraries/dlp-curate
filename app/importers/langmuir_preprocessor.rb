...
class LangmuirPreprocessor
  ...
  def additional_headers
    ['source_row', 'deduplication_key', 'model', 'parent', 'file', 'file_types', 'pcdm_use', 'fileset_label',
     'preservation_master_file', 'intermediate_file']
  end
  ...
  def process_work_filesets_output(work, two_sided, merge_csv)
    work[:filesets].keys.sort.each do |fileset_index|
      ...
      process_fileset_file_and_file_type(fileset)
      merge_csv << fileset
    end
  end
  ...
  def extract_metadata(row, source_row_num)
    ...
    processed_row = CSV::Row.new(additional_headers, [source_row_num, deduplication_key, @work_model_or_type, parent, '', '', ''])
