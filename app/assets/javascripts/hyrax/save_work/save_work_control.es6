...
export default class SaveWorkControl {
  ...
  validateAgreement(filesValid) {
    // Commenting out this.uploads.hasNewFiles since this logic is handled in the ajax call
    // we make while uploading files
    // TODO: Improve styling in _form_files to get this.uploads.hasNewFiles back
    // if (filesValid && this.uploads.hasNewFiles && this.depositAgreement.mustAgreeAgain) {
    if (filesValid && this.depositAgreement.mustAgreeAgain) {
